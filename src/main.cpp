...
int main() {
  ...
  while (g_running) {
    ...
    while ((key = renderer.readInput()) != -1) {
      ...
      case KEY_LEFT:
        camera.rotatePhi(-0.04);
        break;
      case KEY_RIGHT:
        camera.rotatePhi(0.04);
        break;
      case KEY_UP:
        camera.rotateTheta(-0.03);
        break;
      case KEY_DOWN:
        camera.rotateTheta(0.03);
        break;
      ...
      case 'm':
        bh.setMass(std::max(0.2, bh.mass - 0.1));
        disk = AccretionDisk(bh, 10.0);
        break;
      case 'M':
        bh.setMass(std::min(5.0, bh.mass + 0.1));
        disk = AccretionDisk(bh, 10.0);
        break;

      case 'f':
        camera.adjustFOV(-2.0);
        break;
      case 'F':
        camera.adjustFOV(2.0);
        break;
      ...
      case 'e':
        tracer.exposure = std::max(0.5, tracer.exposure - 0.5);
        break;
      case 'E':
        tracer.exposure = std::min(30.0, tracer.exposure + 0.5);
        break;

      case 'c':
      case 'C':
        switch (renderer.color_mode) {
        case ColorMode::ASCII_ONLY:
          renderer.color_mode = ColorMode::COLOR_256;
          break;
        case ColorMode::COLOR_256:
          renderer.color_mode = ColorMode::TRUECOLOR;
          break;
        case ColorMode::TRUECOLOR:
          renderer.color_mode = ColorMode::ASCII_ONLY;
          break;
        }
        break;
      ...
      case 'P':
        ...
        case ColorScheme::RED_ORANGE:
          renderer.color_scheme = ColorScheme::BLUE;
          break;
        case ColorScheme::BLUE:
          renderer.color_scheme = ColorScheme::PURPLE;
          break;
        case ColorScheme::PURPLE:
          renderer.color_scheme = ColorScheme::RED_ORANGE;
          break;
        }
        break;
    ...
    for (unsigned int t = 0; t < num_threads; ++t) {
      threads.emplace_back([&, t, num_threads]() {
        for (int y = t; y < h; y += num_threads) {
          for (int x = 0; x < w; ++x) {
            double u =
                2.0 * static_cast<double>(x) / static_cast<double>(w) - 1.0;
            double v =
                1.0 - 2.0 * static_cast<double>(y) / static_cast<double>(h);

            Vec3 dir = camera.generateRayDirection(u, v, aspect);
            TraceResult result = tracer.trace(cam_pos, dir);

            if (result.hit == HitType::BACKGROUND_STAR &&
    ...
    if (frame_duration.count() < target_ms) {
      std::this_thread::sleep_for(std::chrono::duration<double, std::milli>(
          target_ms - frame_duration.count()));
    }
    ...
    if (renderer.color_mode == ColorMode::COLOR_256)
      color_label = "256-color";
    if (renderer.color_mode == ColorMode::TRUECOLOR)
      color_label = "Truecolor";

    char hud1[256], hud2[256];
    snprintf(hud1, sizeof(hud1),
             " FPS: %.1f | %dx%d | M=%.1f rs=%.1f | "
             "d=%.1f θ=%.2f φ=%.2f | FOV=%.0f° | exp=%.1f | %s%s",
             fps, w, h, bh.mass, bh.rs, camera.cam_distance, camera.cam_theta,
             camera.cam_phi, camera.fov * 180.0 / M_PI, tracer.exposure,
             color_label, tracer.enable_stars ? " ★" : "");

    snprintf(hud2, sizeof(hud2),
             " [←→↑↓] Rotate  [+/-] Zoom  [m/M] Mass  [f/F] FOV  "
             "[e/E] Exposure  [c] Mode  [p] Palette  [s] Stars  [r] Reset  [q] "
             "Quit");
